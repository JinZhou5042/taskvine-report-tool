...
> **Important:** `vine_report` and `vine_export` both depend on data generated by `vine_parse` first.
...
- `vine_report` needs permission to bind a listening port.
...
- You can also copy/download log folders from the cluster to your laptop and run `vine_parse` + `vine_report` locally.
...
Or parse standalone debug files without manually creating `vine-logs/`:
```bash
vine_parse --debug-file /path/to/debug
vine_report
```
If the file is literally named `debug`, the run folder name is derived from the debug timestamp and manager pid, not from the parent directory.

Internally, parsed times are normalized to Unix time. Debug line prefixes are
wall-clock strings and are converted with the numeric `tz_offset` recorded in
the debug log. Unix timestamps embedded in debug messages are already absolute
epoch times and are not timezone-adjusted.
...
Tip: If your cluster node cannot host `vine_report`, copy `vine-run-info/` (or selected run folders with `vine-logs`) to your laptop and run the same commands locally.
...
└── experiment1/
    ...
    ├── csv-files/          # Auto-generated; required by vine_report/vine_export
...
2. Each log collection must contain a complete debug log
...
4. If `vine_report` fails to start on an HPC front-end/login node, check cluster policy and firewall rules for local port binding. When in doubt, use `vine_export` and open the generated standalone HTML report instead.
