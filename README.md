...
After installation, you can use `vine_parse`, `vine_report`, and `vine_export` directly from anywhere.
...
- `--max-tasks`: Upper bound for task bars in Task Execution Details
...
After parsing and exporting, each experiment may have these generated directories:
```
vine-run-info/
└── experiment1/
    ...
    ├── png-files/          # Static plot exports (generated by vine_export)
    │   └── ...             # Auto-generated from selected --sections
    ├── html-files/         # Standalone shareable report (generated by vine_export)
    │   └── <template>.html
...
- **`png-files/`**: Contains static chart images exported by `vine_export`. These are useful for slide decks, docs, and quick sharing.

- **`html-files/`**: Contains a single standalone HTML report generated by `vine_export`. The images are embedded directly in the HTML, so the file can be copied and opened anywhere without extra assets.
